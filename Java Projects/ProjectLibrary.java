...
public class ProjectLibrary extends JFrame implements 
...
cdcodeTF,cdtitleTF,cdisbnTF,cdeditionTF,cdpubTF;
	Container c;
...
menuItemN,menuItemB,menuItemC,menuItemH,menuItemA,menuIte
...
mE;
	...
	public Project4()
	{
		...
		c = getContentPane(); c.setBackground(new 
...
lcity.setForeground(Color.white);
		JLabel pad = new JLabel("Permenent Address"); 

pad.setForeground(Color.white);

		        JLabel leadd = new JLabel("Email 

Address"); leadd.setForeground(Color.white);
...
ImageIcon("backg.jpg");
			...
			ImageIcon college = new 

ImageIcon("Movie1.PNG");
			JLabel colLogo = new JLabel(college);

		
			ImageIcon LineStraight = new 
...
date.setForeground(Color.black); date.setBackground(Color.white);
		phno = new JTextField(); 

phno.setForeground(Color.black); phno.setBackground(Color.white);
		name = new JTextField(); 

name.setForeground(Color.black); name.setBackground(Color.white);
		add = new JTextField();  

add.setForeground(Color.black); add.setBackground(Color.white);
...
update.setBackground(Color.pink);
			...
			search.setToolTipText("Search the 

record");	
			save.setToolTipText("Save the 

record");	
			delete.setToolTipText("Delete the 

record");	
			exit.setToolTipText("Exit from the 

library");	
			update.setToolTipText("Update the 
...
st.executeQuery("select * from Directory where Id="+a);
			                     while(rs.next())
				    {		
...
System.out.println(sql);
	...
	{	
		...
		if(source==menuItemA)
		{
			JFrame aboutus = new JFrame("About 
...
JFrame("Book's available in library");
...
JButton("Search", new ImageIcon("SEARCH.PNG"));
...
JButton("Save", new ImageIcon("SAVE.PNG"));
...
JButton("Update", new ImageIcon("UPDATE.PNG"));
...
c.prepareStatement("Insert into Book values(?,?,?,?)");
...
ps=c.prepareStatement ("Update Book set BookName=? , 

AuthorName=? , Price=? where BookCode="+bcodeTF.getText() ); 
...
JFrame("CD's available in library");
				cd.setSize(510,540);	
...
cd.getContentPane().add(cdlogo);  
				...
				cdcode = new JLabel("CD 

Code:"); cdcode.setForeground(Color.white);
				cdtitle = new JLabel("Title:"); 

cdtitle.setForeground(Color.white);
				cdisbn = new JLabel("ISBN 

NO:"); cdisbn .setForeground(Color.white);		
				cdedit = new 

JLabel("Edition:");	cdedit .setForeground(Color.white);
				cdpub = new 

JLabel("Publication:");cdpub .setForeground(Color.white);
...
JButton("Delete", new ImageIcon("DELETE.PNG"));
				cdupdate = new 

JButton("Update", new ImageIcon("UPDATE.PNG"));
				cdexit = new JButton("Exit", 
...
st.executeQuery("select * from CD  where 
...
c.prepareStatement("Insert into CD values(?,?,?,?,?)");
...
ps=c.prepareStatement("Delete from CD where 
...
ps=c.prepareStatement ("Update CD set Title=?, IsbnNO=?, 

Edition=?, Publication=?  where CdCode="+cdcodeTF.getText());
...
st.executeQuery("select * from Book where 
...
from Directory where Id="+id.getText());
...
c.prepareStatement("Insert into Directory 
...
ps=c.prepareStatement("Delete from Directory where 
...
ps=c.prepareStatement ("Update Directory set  

Date=?,Name=?,Class=?,Shift=?,Progress=?,Address=?,City=?,Perme

nent_Address=?,PhoneNo=?,eaddress=?  where Id="+id.getText());
		PreparedStatement ps=c.prepareStatement 

("Update Directory set Name=?, Class=?, Shift=?, Progress=?, 

Address=?, City=?, Permenent_Address=?, PhoneNo=?, eaddress=?, 

DateToday=? where Id="+id.getText());
